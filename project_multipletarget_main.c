...
int targetpos = 0;
int targetposn = 0; // Publishtargetpos에서 targetpos를 수정하기 위해 추가한 변수
int targetposarray[5] = {};
...
int count = 0; // count 전역변수로 설정해서 SubscribeClock에서 수정할 수 있도록 변경.
unsigned int checkTime, checkTimeBefore, deltaTime, startTime, startTimeBefore, startTime0;
...
void saveGraph() // 그래프 저장하는 함수
{
    ...
    fprintf(gnuplot, "set yrange [-6:6]\n");
...
{
    dataArray[dataIndex][0] = (float)(checkTime - startTime0)/1000000.0;
...
void PublishTargetPos(int *targetposn, int new_targetpos) 
{
    *targetposn = new_targetpos;
    targetpos = *targetposn;
    printf("target position : %d\n",targetposn);

    // targetpos 변경 시 변수 초기화 및 선언
    Integral = 0;
    startTime = checkTime; // ITAE를 구하기 위해 startTime을 현재시간(타겟포즈가 할당된 시간)으로 변경
    flag = 1;
    count = k;
    if (count > 4) count = 4; // count 4 초과시 4로 변경
}
...
void SubscribeClock(void) // clock이 rising edge일때마다 호출되는 함수
{
    ...
    if (i == 3){
        ...
        if (get_pos[0] == get_pos[1] && get_pos[1] == get_pos[2]){ // 3개 원소가 모두 같을 때
            ...
            PublishTargetPos(&targetposarray[k], store_pos[k]); // target position 반환함수 실행
            k++;
        }
    }
    else{
...
void PID() // PID 제어값 계산하는 함수
{
    ...
    if (deltaTime > 0) {
        ...
        // 적분 계산
        Integral += deltaTime/1000000.0 * errorPosition;
...
int main() 
{
    ...
    while(1) { 
        ...
        if (checkTime - checkTimeBefore > T) { // 모터 움직이는 함수 (수업시간 참조)
            ...
            if(flag == 1&&(fabs(errorPosition)<fabs(targetpos*0.1))){ // par는 90% rising time
                par[count] = (checkTime - startTime);
                flag = 0;
            }

            if((checkTime - startTime >= par[count]) && (checkTime - startTime) < par[count] + 4000000){ //ITAE
                ITAE[count] = ITAE[count] + (checkTime-startTime-par[count])/1000000.0*abs(errorPosition)*0.005; // (checkTime-startTime-par)를 곱해줘야되지 않을까요??
            }
