    ...
    while(1) { 
        ...
        if (checkTime - checkTimeBefore > T) { // 모터 움직이는 함수 (수업시간 참조)
            ...
            if(flag == 1&&(errorPosition<targetpos*0.1)){ // par는 90% rising time
                par = checkTime - startTime;
                flag = 0;
            }

            if((checkTime - startTime >= par) && (checkTime - startTime) < par + 4000000){ //ITAE
                ITAE = ITAE + 0.005*abs(errorPosition); // (checkTime-startTime-par)를 곱해줘야되지 않을까요??
            }
